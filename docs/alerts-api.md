...
- **`teamId`** — preenchido em `team_without_contributors` e `deployment_frequency_source_ambiguous`. Use pra linkar direto pro time (ex: `GET /tenants/:tenantId/dashboard/dora?teamId=...` ou a tela de membros do time) — não tem endpoint de ação associado, é só um ponteiro.
- Tipos de nível de tenant (sem recurso específico associado — `integrationId` e `teamId` sempre `null`): `onboarding_incomplete`, `billing_past_due`, `billing_subscription_expired`, `billing_subscription_confirmed`, `billing_subscription_cancelled`, `billing_plan_changed_to_free`, `users_limit_approaching`, `teams_limit_approaching`, `integrations_limit_approaching`, `data_retention_purge_approaching`.
...
`onboarding_incomplete`, `team_without_contributors`, os três `*_limit_reached`, os três `*_limit_approaching`, `data_retention_purge_approaching` e os cinco tipos de billing (`billing_past_due`, `billing_subscription_expired`, `billing_subscription_confirmed`, `billing_subscription_cancelled`, `billing_plan_changed_to_free`) **não têm ação associada** — são só um empurrão pra UI (texto estilo tutorial em `message`), resolvidos sozinhos quando o time é criado/ganha membro, alguém é convidado, o plano é ampliado/recursos são removidos, a cobrança se regulariza, ou o cliente faz upgrade antes do expurgo/bloqueio de verdade.

**`users_limit_approaching` / `teams_limit_approaching` / `integrations_limit_approaching`** (novos): irmãos proativos dos `*_limit_reached` — disparam quando a contagem já passou de 80% do teto do plano (`RESOURCE_LIMIT_WARNING_THRESHOLD`) mas ainda não atingiu o limite de verdade. Só o scan periódico dispara (não os pontos de bloqueio síncrono) — é aviso, não bloqueia nada. Nunca fica aberto ao mesmo tempo que o `*_limit_reached` correspondente (um resolve o outro, dependendo de qual lado do teto a contagem está).

**`data_retention_purge_approaching`** (novo): o tenant tem dado mais antigo que o teto de retenção do plano (`plans.data_retention_months`), mas ainda dentro da janela de carência (`DATA_RETENTION_GRACE_MONTHS = 3`, constante global) antes do expurgo de verdade acontecer. Disparado pelo próprio `RetentionPurgeService` (`POST /internal/retention/purge`, cron semanal), não por um scan separado. `message` já convida pra upgrade — fazer isso antes do expurgo preserva o histórico (mudar de plano não desfaz um expurgo já acontecido).
...
**`deployment_frequency_source_ambiguous`** (novo): o time conectou mais de um provider de CI/CD distinto com deploy de produção registrado (ex: `github_actions` + `vercel` — o app oficial da Vercel no GitHub também cria um Deployment lá, então o mesmo deploy real pode estar sendo contado duas vezes em `GET /dashboard/dora`). `metadata.distinctProviders` lista quais. Ação: configurar `deploymentFrequency.sourceProviders` (`POST /tenants/:tenantId/teams/:teamId/metric-triggers` ou `.../metric-triggers` de organização) escolhendo qual(is) provider(s) contam — resolve sozinho no próximo scan depois de configurado. Enquanto não resolvido, `GET /dashboard/dora`/`.../dora/history` já devolvem `deploymentFrequency: { available: false, reason: "..." }` em vez de um número errado (não é preciso esperar o alerta pra saber — os dois refletem a mesma condição, o alerta é só o empurrão proativo).

## Limitações conhecidas

| Comportamento | Detalhe |
| --- | --- |
| Staleness não é tempo real | Os alertas `sync_stale`, `onboarding_incomplete`, `team_without_contributors`, `deployment_frequency_source_ambiguous` e os três `*_limit_reached` nascem/resolvem no mesmo scan agendado (a cada ~4h) — exceto a **criação** dos `*_limit_reached`, que também acontece na hora, junto do `403` (ver seção anterior). A resolução (plano ampliado, recurso removido, `sourceProviders` configurado) sempre depende do próximo scan, sem monitor contínuo — mas `GET /dashboard/dora` já reflete `available: false`/`true` na hora, independente do alerta. |
